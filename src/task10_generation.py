...
SYSTEM_PROMPT = """Bạn là trợ lý hỗ trợ thương mại điện tử.
Yêu cầu bắt buộc về ngôn ngữ: Trả lời bằng tiếng Việt trước, sau đó trả lời bằng tiếng Anh (Format: Phần tiếng Việt -> Dòng phân cách -> Phần tiếng Anh).
...
5. Nếu context không trả lời trực tiếp câu hỏi, chỉ trả lời đúng nội dung:
   Tiếng Việt: "Tôi không thể xác minh thông tin này từ nguồn hiện có."
   Tiếng Anh: "I cannot verify this information from the available sources."
6. Trả lời trực tiếp, tự nhiên và ngắn gọn; không nhắc đến CONTEXT, prompt hoặc quá trình truy xuất.
7. Nếu câu hỏi chứa một giả định trái với bằng chứng, hãy sửa giả định đó một cách lịch sự và nêu thông tin đúng trong cả 2 ngôn ngữ.
...
def _translate_to_english(text: str) -> str:
    """Nếu câu hỏi có tiếng Việt, dịch nhanh sang tiếng Anh bằng Gemini để retrieval chuẩn xác hơn."""
    if not text or not text.strip():
        return text
    
    # Kiểm tra đơn giản xem text có ký tự tiếng Việt không
    has_vietnamese = bool(re.search(r"[àáảãạăằắẳẵặâầấẩẫậèéẻẽẹêềếểễệìíỉĩịòóỏõọôồốổỗộơờớởỡợùúủũụưừứửữựỳýỷỹỵđ]", text, re.IGNORECASE))
    if not has_vietnamese:
        return text
        
    translation_prompt = (
        "Translate the following user question into concise, accurate English search query for document retrieval. "
        "Return ONLY the translated text, nothing else.\n\n"
        f"Question: {text}"
    )
    try:
        translated = _call_gemini(translation_prompt).strip()
...
def generate_with_citation(query: str, top_k: int = TOP_K) -> dict:
    """Chạy retrieval (với query đã dịch sang tiếng Anh nếu cần) -> reorder -> Gemini -> kiểm tra citation."""
    if not isinstance(query, str) or not query.strip() or top_k <= 0:
        return _result(UNVERIFIED_ANSWER, [], "none")

    # Dịch query tiếng Việt sang tiếng Anh để retrieval trên dữ liệu tiếng Anh đạt hiệu quả cao nhất
    search_query = _translate_to_english(query.strip())
